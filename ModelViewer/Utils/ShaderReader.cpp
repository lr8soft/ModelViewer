...
using namespace std;
void GLUtils::ShaderReader::show_failed_info(GLuint temp_shader,string sre,int type)
{
	GLsizei len;
	GLchar* log = nullptr;
	switch (type) {
...
const GLchar * GLUtils::ShaderReader::read_from_file(const char* path)
{
	GLchar *ret = nullptr;
	ifstream io(path);
	if (!io) {
...
GLUtils::ShaderReader::ShaderReader()
{
	temp_program = glCreateProgram();
}

GLUtils::ShaderReader::ShaderReader(GLuint program)
{
	temp_program = program;
}

GLboolean GLUtils::ShaderReader::loadFromFile(const char *path, GLenum type)
{
...
GLboolean GLUtils::ShaderReader::loadFromInfo(const char *info,GLenum type)
{
...
GLboolean GLUtils::ShaderReader::addNewShader(GLuint shader, GLenum type)
{
...
GLboolean GLUtils::ShaderReader::linkAllShader()
{
	if (shader_count>0) {
...
GLuint GLUtils::ShaderReader::getProgramHandle()
{
	return temp_program;
}
